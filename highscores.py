    ...
    while True:  # wait for user to acknowledge and return
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key in [pygame.K_RETURN, pygame.K_KP_ENTER]:
                os.startfile(os.path.join(os.getcwd(),constants.MAIN_MENU))
                sys.exit()
...
def highscore(screen, file_name, your_points):
    ...
    if your_points > high_score:
        your_name = enterbox(screen, "YOU GOT THE NEW HIGHSCORE " + str(your_points) +" - What is your name?")

    elif your_points == high_score:
        your_name = enterbox(screen, "YOU HAVE MATCHED THE HIGHSCORE " + str(your_points) +" - What is your name?")

    elif your_points < high_score:
        st1 = "Current highscore "
        st2 = " made by "
        st3 = ". Your score: "
        st4 = " . What is your name?"
        txt = st1 + str(high_score) + st2 + high_name + st3 + str(your_points) + st4
